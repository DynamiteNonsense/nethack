/* Modified 2026-09-09 for RU display preview 0.1.
 * See ru-project/README_PREVIEW_RU.md for scope and limitations.
 */
...
#if defined(WIN32CON) && !defined(NO_VT)
#include "nhr_messages.h"
#define NHR_TTY_PREVIEW 1
#endif
...
update_topl(const char *bp)
{
    char *tl, *otl;
    int n0;
    int notdied = strncmp(bp, "You die", 7);
    struct WinDesc *cw = wins[WIN_MESSAGE];
    boolean skip = (cw->flags & (WIN_STOP | WIN_NOSTOP)) == WIN_STOP;
#ifdef NHR_TTY_PREVIEW
    boolean ru_message = (nhr_lookup(bp) != NULL);
    boolean ru_previous = (nhr_lookup(gt.toplines) != NULL);
#endif
...
#ifdef NHR_TTY_PREVIEW
         && !ru_message /* render the original catalog key without alteration */
#endif
         ; ) {
        otl = tl;
...
#ifdef NHR_TTY_PREVIEW
static void
nhr_tty_emit(unsigned codepoint, void *context)
{
    (void) context;
    /* Windows xputc(int) accepts these BMP codepoints. Cyrillic is above
     * the byte/codepage range, so consoletty.c writes native Unicode. */
    topl_putsym((int) codepoint);
}
#endif
...
{
#ifdef NHR_TTY_PREVIEW
    const char *translated = nhr_lookup(str);
    if (translated) {
        nhr_render(translated, CO - 1, ttyDisplay->curx,
                   nhr_tty_emit, NULL);
        return;
    }
#endif
