...
attempt_restore:
    ...
    /* RU preview diagnostic. Does not alter hero data or consume a turn. */
    {
        const char *ru_test = getenv("NETHACK_RU_TEST");
        if (ru_test && !strcmp(ru_test, "1") && WINDOWPORT(tty)) {
            pline("Cyrillic test: hedgehog, pike, eat some more.");
            pline("Russian preview: this long message checks wrapping without losing the English original.");
            pline("Russian preview: test complete.");
        }
    }
#endif
    moveloop(resuming);
